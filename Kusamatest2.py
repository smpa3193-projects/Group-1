...
all_words = []
infinite_tweets = twitter.search(q='#InfiniteKusama', count="500")
yayoi_tweets = twitter.search(q='#YayoiKusama', count="500")
text = []
...
with open ('yayoikusama.csv', 'w') as fp:
    ...
    for result in yayoi_tweets['statuses']:
        try:
            ...
            url = None
        text.append(['#YayoiKusama', result['text'].encode('utf-8'), url])
    a.writerows((text))
